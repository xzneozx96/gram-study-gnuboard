...
<form name="fsearch" method="get" class="local_sch01 local_sch">
	...
	<select name="sfl" id="sfl">
		...
		<option value="wr_name,1"<?php echo get_selected($sfl, 'wr_1,1'); ?>>연락처</option>
	</select>
...
<input type="hidden" name="sw" value="">
	...
	<div id='bo_list' class="tbl_head01 tbl_wrap">
			...
			<tr>
					...
					<th scope="col">예상창업비용</th>
					<th scope="col">점포보유유무</th>
			...
			<tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
					...
					<td class="td_subject"><a href="<?php echo $list[$i]['href'] ?>"><?php echo $list[$i]['name'] ?></a></td>
					<td class="td_subject"><?php echo hyphen_tel_number($list[$i]['wr_1']) ?></td>
					<td class="td_subject"><?php echo $list[$i]['wr_email'] ?></td>
					<td class="td_subject"><?php echo $list[$i]['wr_2'] ?></td>
					<td class="td_subject"><?php echo $list[$i]['wr_3'] ?></td>
					<td class="td_subject"><?php echo $list[$i]['wr_4'] ?></td>
					<td class=""><?php echo cut_str(trim(strip_tags($list[$i]['wr_content'])), 44)?></td>
					<td class="td_datetime"><?php echo $list[$i]['datetime2'] ?></td>
					<td class="td_wr_5<?php if($list[$i]['wr_5'] == '상담완료') echo ' success';?>"><?php echo ($list[$i]['wr_5'] == '상담완료')? $list[$i]['wr_5'] : '상담대기' ?></td>
			</tr>
			<?php } ?>
	...
	<div class="btn_fixed_top">
			<?php if ($list_href || $write_href) { ?>
				<?php if ($is_checkbox) { ?>
				<input type="submit" name="btn_submit" value="상담완료" onclick="document.pressed=this.value" class="btn_02 btn">
...
function fboardlist_submit(f) {
    ...
    if(document.pressed == "상담완료") {
        if (!confirm("선택한 게시물을 상담완료 하시겠습니까?"))
            return false;
